...
def safe_hex(a):
  return a if not isinstance(a, (int)) else hex(a)

# compare the addrs between two logs
def addr_diff(log1, log2, merge):
  for bus in [0, 1, 2, 128, 129, 130]:
    if merge:
      if bus >= 128: break
      addrs1 = sorted(log1.get_addrs_on_buses([bus, bus+128]).keys())
      addrs2 = sorted(log2.get_addrs_on_buses([bus, bus+128]).keys())
    else:
      addrs1 = sorted(log1.get_addrs_on_buses([bus]).keys())
      addrs2 = sorted(log2.get_addrs_on_buses([bus]).keys())
...
if __name__ == "__main__":
  if len(sys.argv) < 2:
    ...
    print("add --mergeop arg to merge openpilot/panda buses with normal buses")
    exit(-1)
  ...
  if "--bits" in sys.argv:
    bit_diff(log1, log2, "--mergeop" in sys.argv)
  else:
    addr_diff(log1, log2, "--mergeop" in sys.argv)
